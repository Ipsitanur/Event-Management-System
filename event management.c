...
switch(choice){
 ...
 case 4:
        ...
        fscanf(fp,"%s %s %s %s",customer.name,customer.username,customer.password,customer.phone);
        fclose(fp);
...
if(fp==NULL){
 ...
 printf ("\n---Customer List---\n");
 for(;fscanf(fp,"%s %s %s %s",c.name,c.username,c.password,c.phone)!=EOF;){
...
for(;fscanf(fp,"%s %f %f %f %f" ,b.username,&b.EventCost,&b.ServiceCost,&b.VAT,&b.totalAmount)!=EOF;){
